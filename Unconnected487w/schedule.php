...
<?php
  ...
  $sql = "SELECT work_day, start_time, stop_time, store_id FROM schedule WHERE employee_id = '{$id}' ORDER BY work_day";
...
//  debug_to_console($schedule);
  ...
  foreach($schedule as $day) { ?>
    <tr>
    <th scope="row">
      <?php echo $day['work_day']; ?>
    </th>
    <td>
      <?php echo $day['start_time']; ?>
    </td>
    <td>
      <?php echo $day['stop_time']; ?>
    </td>
    <td>
      <?php echo $day['store_id'];?>
    </td>
    <td>
      <?php echo ($conn->query("SELECT store_location FROM store WHERE store_id = {$day['store_id']};"))->fetch_assoc()['store_location']; ?>
    <td>
    </tr>
    <?php
  }
